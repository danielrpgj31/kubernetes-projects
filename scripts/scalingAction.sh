...
operator_service_name="internal-weblogic-operator-service"
...
then
    ...
    echo "    operator_service_name - WebLogic Operator Service name, default=internal-weblogic-operator-service"
...
if [ -e $pem_filename ]
then
  result=`curl -k -X POST -H "$content_type" -H "$requested_by" -H "$authorization" -d "$request_body" $operator_url`
else
